...
void InitSnake(PSnakeInfo ps)
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		{
			ps->pSnakeHead = newnode;
		}
		else
		{
			newnode->next = ps->pSnakeHead;
			ps->pSnakeHead = newnode;
		}
	}
...
again:
	...
	PSnakeNode newnode = (PSnakeNode)malloc(sizeof(SnakeNode));//创建食物
	if (newnode == NULL)
	{
...
{
	if (ps->pSnakeHead->x == 0 || ps->pSnakeHead->x == 56 ||
...
{
	PSnakeNode pcur = ps->pSnakeHead->next->next;
	while (pcur->next)
	{
...
void GameRun(PSnakeInfo ps)
{
	...
	do {
		SetPos(64, 10);
		printf("总分 = %2d  ", ps->Score);
		printf("每个食物分数 = %2d  ", ps->fScore);
